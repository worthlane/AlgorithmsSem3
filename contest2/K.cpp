...
bool x_comparator(const Point &a, const Point &b)
{
    return a.x < b.x;
}
...
{
    auto it = std::lower_bound(points.begin(), points.end(), Point{x, 0}, x_comparator);
...
class Line
{
    ...
    bool operator==(const Line& other) const { return k == other.k && b == other.b; }
    bool operator>=(const Line& other) const { return k == other.k && b >= other.b; }
    bool operator<=(const Line& other) const { return k == other.k && b <= other.b; }
    ...
    public:
        ...
        void add_line(const ll b, const ll k)
        {
            ...
            if (!lines.empty() && !points.empty())
            {
                ...
                while (prev_line.eval(prev_point.x) > new_line.eval(prev_point.x) && !lines.empty())
                {
