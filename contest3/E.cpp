...
void dfs_recursive(std::vector<std::vector<ll>>& graph, const ll start, std::vector<Color>& colors, std::vector<ll>& order)
{
    colors[start] = GRAY;
...
void dfs_fill_component(std::vector<std::vector<ll>>& graph, const ll start, std::vector<Color>& colors, std::vector<ll>& components, const ll curr_component)
{
    colors[start] = GRAY;
    for (size_t neighbour : graph[start])
    {
...
int main()
{
    ...
    for (ll i = N - 1; i >= 0; i--)
    {
        ...
        {
            dfs_fill_component(inverse_graph, elem, inverse_colors, components, curr_component);
            curr_component++;
        }
    }
